...
- [ ] **Three gates in `brickwright-lite` open something with no floor under it.**
  Outside this worktree's write boundary, so recorded rather than repaired. Read,
  not merely flagged — a fourth suspect, `starter-journeys.test.mjs`, was cleared
  by reading it: `assert.deepEqual(journeys.map(j => j.id), ['circuit','board','lego'])`
  is a floor, and the screen has been taught to see that shape.
  - `no-dead-overlay-modules.test.mjs` — `walk()` recurses the overlay tree with no
    floor, and all three verdicts are `deepEqual(…, [])` plus one `<=` ceiling. An
    empty walk satisfies every one of them and would report every module as live.
    This is the file whose CI guard already caps the dead-module waiver list because
    it grew 5 → 16 while staying green; an empty walk is the same failure by a
    shorter route.
  - `stc12-pinmap.test.mjs` — the ghost-pin scan reads
    `readdirSync(resolve(dir), {recursive: true})` with no floor, so an empty walk
    finds no ghost pins and passes. The datasheet assertions in the same file are
    direct and unaffected; it is the scan that is vacuous, not the file.
  - `debug-target-routing.test.mjs` — **all seven tests** are gated on
    `existsSync(packages/scratch-gui/src/lib/bw-board/debug-target-factory.js)`,
    which is generated by `npm run integrate` into a gitignored tree. When
    integration is skipped or stale the whole file reports `SKIP: not integrated`,
    and a skip reads as a deliberate exclusion — the wave-1 shape exactly. This same
    repo already documents `packages/` going stale and producing a confident wrong
    answer (`no-dead-overlay-modules`, 2026-08-10, six stale files). The fix is the
    asymmetric guard: skip locally, FAIL in CI, where integrate is supposed to have
    run.
